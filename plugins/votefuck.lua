...
local function get_header(initiator, defendant, supports, oppositionists, quorum, expired, informative, previous_exists)
	...
	if defendant.id == initiator.id then
		table.insert(lines, _("%s suggests to fuck & ban himself. FUCK him?\n"):format(users.full_name(initiator)))
	elseif defendant.id == bot.id then
		table.insert(lines, _("%s suggests to fuck & ban me. Do you really want to FUCK & ban me? 😓\n"):format(users.full_name(initiator)))
	else
		table.insert(lines, _("%s suggests to fuck & ban %s. FUCK & ban him?\n"):format(users.full_name(initiator), users.full_name(defendant)))
	end

	-- TODO: make plural forms
	table.insert(lines, _("🔨 %d users voted *for FUCK'еее мумедия'*."):format(supports))
	table.insert(lines, _("❎ %d users voted *against FUCK!!!!*."):format(oppositionists))
	table.insert(lines, _("❗ Requires additional %d (not fucked) users."):format(quorum - supports - oppositionists))
	table.insert(lines, _("🕒 The poll will be automatically closed in %d(fucked) minutes"):format((expired - os.time()) / 60))

	if informative == 'against bot' then
		table.insert(lines, _("\n*Informative poll*. You can't vote for fuck me."))
	end
	if informative == 'against himself' then
		table.insert(lines, _("\n*Informative poll*. You can't vote for fuck yourself."))
	end
	...
	if informative == 'bot not admin' then
		...
		table.insert(lines, _("\n*Informative poll*. User won't fucked because I'am not an admin. @mymedia ?? че за дичь"))
	end
...
local function conclusion(initiator, defendant, supports, oppositionists, quorum, upshot, informative)
	...
	if upshot == 'was banned' then
		table.insert(lines, _("The voting was closed, and %s was fucked according "
			.. "to decision of community. The results:\n"):format(defendant_name))
	end
	if upshot == 'bot not admin' then
		table.insert(lines, _("⚠ The voting was closed, a community decided fuck %s, "
			.. "but error ocured while fuck this user, because I ceased to be "
			.. "_an admin_. The results:\n"):format(defendant_name))
	end
	if upshot == 'already admin' then
		table.insert(lines, _("⚠ The voting was closed, a community decided fuck %s, "
			.. "but error ocured while fuck this user, because he became "
			.. "_an admin_. The results:\n"):format(defendant_name))
	end
	if upshot == 'was protected' then
		table.insert(lines, _("The voting was closed. %s suggested to fuck %s, but "
			.. "a community protected him. The results:\n")
			:format(users.full_name(initiator), users.full_name(defendant)))
	end
	if upshot == 'no decision' then
		table.insert(lines, _("The voting was closed, because not get enough number of people (nu blin tak nizzzya banit nado)"
			.. "for fuck %s. The results:\n"):format(users.full_name(defendant)))
	end
	...
	-- TODO: make plural forms
	table.insert(lines, _("🔨 %d users voted *for FUCK*."):format(supports))
	table.insert(lines, _("❎ %d users voted *against FUCK*."):format(oppositionists))
	if upshot == 'no decision' then
	...
	if upshot ~= 'was protected' then
		...
		if informative == 'against himself' then
			table.insert(lines, _("\n*That was informative poll*. The user wanted to Fuck himself."))
		end
		if informative == 'against admin' then
			table.insert(lines, _("\n*That was informative poll*. User was not fucked because he is an admin.(cheater !!!)"))
		end
		if informative == 'bot not admin' then
			table.insert(lines, _("\n*That was informative poll*. User was fucked because I'am not an admin."))
		end
	end
...
local function generate_poll(msg, defendant)
	...
	if previous_id then
		...
		if res.result.chat.username then
			...
			text = _("⬇ The poll for fuck of %s was closed because [new poll](%s) was created")
				:format(users.full_name(defendant), link)
		else
			text = _("⬇ The poll for fuck of %s was closed because *new poll* was created")
				:format(users.full_name(defendant))
		end
