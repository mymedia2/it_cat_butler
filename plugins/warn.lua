local function doKeyboard_warn(user_id, ln)
	local keyboard = {}
    keyboard.inline_keyboard = {
    	{
    		{text = _("Reset warns", ln), callback_data = 'resetwarns:'..user_id},
    		{text = _("Remove warn", ln), callback_data = 'removewarn:'..user_id}
    	}
    }
    return keyboard
end
...
local function action(msg, blocks)
    ...
    if blocks[1] == 'resetwarns' and msg.cb then
		...
		local text = _("The number of warns received by this user has been *reset*\n"
			.. "`(Admin: %s)`", msg.ln):format(msg.from.first_name:mEscape())
	...
	if blocks[1] == 'removewarn' and msg.cb then
		...
		if tonumber(num) < 0 then
			text = _("The number of warnings received by this user is already _zero_", msg.ln)
			...
			text = _("*Warn removed!*\n"
				...
				.. "_Max allowed_ *%d*", msg.ln):format(tonumber(num), tonumber(nmax))
		end
    ...
    if not msg.reply then
        api.sendReply(msg, _("Reply to a message to warn the user", msg.ln))
	    return
	end
	--return nil if a mod is warned
	if roles.is_admin(msg.reply) then
		api.sendReply(msg, _("A moderator can't be warned", msg.ln))
	    return
	end		
    ...
    if blocks[1] == 'warn' then
		...
		if num >= nmax then
			...
			--try to kick/ban
			if type == 'ban' then
				text = _("User %s *banned*: reached the max number of warnings (%d / %d)", msg.ln):format(name:mEscape(), num , nmax)
				...
				text = _("User %s *kicked*: reached the max number of warnings (%d / %d)", msg.ln):format(name:mEscape(), num , nmax)
		    ...
		    if not res then
		    	...
		    	if type == 'ban' then --add to the banlist
		    		local why = _("Too many warnings", msg.ln)
			...
			text = _("%s *has been warned*.\n"
				.. "_Number of warnings_ *%d*\n"
				.. "_Max allowed_ *%d*", msg.ln):format(name:mEscape(), num, nmax)
			local keyboard = doKeyboard_warn(msg.reply.from.id, msg.ln)
